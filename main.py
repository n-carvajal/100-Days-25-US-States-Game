...
states_guessed_df = pd.read_csv("states_guessed.csv")
states_remaining_df = pd.read_csv("missing_states.csv")
...
if len(states_guessed) == 0:
    label.clear()
    states_guessed = []
    states_remaining_df = pd.read_csv("50_states.csv")
    states_remaining = states_remaining_df["state"].to_list()
else:
    if new_game():
        label.clear()
...
while len(states_guessed) < len(states_list):
    ...
    if guess_attempts % 10 == 0:
        ...
        if end_game():
            ...
            states_guessed.to_csv("states_guessed.csv")
